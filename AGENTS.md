...
- **别名**：会话表加**别名**字段，显示优先级 `别名 > 平台名 > ID`；别名由用户/管理员通过指令设置并持久化（群号抽象，别名是唯一可让用户自己控制的显示方案）。
- 注册**两个工具**配合使用：
  1. `list_sessions`：返回人类可读的会话清单（序号/名称/最后活跃时间）
  2. 投递工具接收 `target`（人名/群名/序号）而非 UMO，插件内部解析为 UMO
- **重命名走指令而非 LLM 工具**：改名是用户操作，不该让 LLM 调用。用 `@filter.command("courier_rename")` 指令（`/courier_rename <目标> [别名]`，不填别名=清除），管理员权限由 `require_admin` 开关在 handler 内判断（`event.role != "admin"`），与投递工具共享 target 解析逻辑，同样校验目标属于当前机器人，写回会话表并持久化。
